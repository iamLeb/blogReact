    ...
    const deleteCategory = async (id) => {
        ...
        if (confirmation) {
            try {
                ...
                await fetchCategories(); // Ensure this is awaited if it's an async function
            } catch (error) {
                ...
                <div className="overflow-x-auto">
                    <table className="table-auto w-full">
                        ...
                        <tr className="text-center text-sm bg-gray-100">
                            <th className="px-4 py-2">CatID</th>
                        ...
                        {categories.map((category) => (
                            <tr key={category._id} className="text-center text-xs border-b">
                                <td className="px-4 py-2">{category._id.substring(18, category._id.length - 2)}</td>
                                <td className="px-4 py-2">{category.name}</td>
                                <td className="px-4 py-2">{format(category.createdAt)}</td>
                                <td className="px-4 py-2">
                                    <div className="flex sm:block">
                                        <button className="px-2 py-1 rounded bg-green-500 text-white">Edit</button>
                                        <button onClick={() => deleteCategory(category._id)} className="ml-2 px-2 py-1 rounded bg-red-500 text-white">Remove</button>
                                    </div>
                                </td>
                            </tr>
                        ))}
